...
#am <- am[1:10,1:8]  # first 10 rows and first 8 columns, for example
...
#plot(gbn, vertex.color = "cornflowerblue", vertex.size = 10, layout = layout_nicely)

set.seed(3)
par(mfrow=c(1,2))
# to color node by type
 plot(gbn, vertex.color = V(gbn)$type+1, vertex.size = V(gbn)$type*5+5, layout = layout_nicely, vertex.label.dist = 2)
# layout as bipartite
 plot(gbn, vertex.color = V(gbn)$type+1, vertex.size = V(gbn)$type*5+5, layout = layout_as_bipartite(gbn), vertex.label.dist = 2, vertex.label.cex = .4)
```

This network contains two types of nodes. It is therefore called a *bipartite network*. One type is individuals. The other type is the affiliation (or interest). Ideally, we would like a network of people who share edges if they have shared interests. That is, we would like a *monopartite network* that only contains individuals who are connected by shared interests.
...
# The function to project.  Multiplicitity = TRUE will produce weighted edges between people that represent the number of affiliations they share (values that are both > 0). 
...
```{r threshold}
# thresh_graph is a function I wrote to remove edges below a threshold and change the weights of all remaining edges to 1. Inputs are a graph and a threshold value. You never need to change the code below.
...
# add attr="weight" if you want to see weights
...
uploadedg <- igraph::graph_from_adjacency_matrix(uploadedam, mode="undirected", weighted=TRUE)

# plot 
plot(uploadedg, vertex.label.size=2)
...
par(mfrow=c(1,1))
# plot
plot(cnetwork3, vertex.size = igraph::degree(cnetwork3)*2)
...
plot(cnetwork3, edge.width=E(cnetwork)$weight, layout=layout_with_kk(cnetwork, weight=1/E(cnetwork)$weight))
# layout with fr uses weights by default to pull nodes together
plot(cnetwork3, edge.width=E(cnetwork)$weight, layout=layout_with_fr)
```
...
par(mfrow=c(1,1))
# plot
plot(cnetwork3, vertex.size = igraph::transitivity(cnetwork3, type="local", isolates = "zero")*10, vertex.label.dist = 2)
...
par(mfrow=c(1,1))
# plot
plot(cnetwork3, vertex.size = igraph::betweenness(cnetwork3)/2+2, vertex.label.dist = 2)
...
```{r closeness}
# compute closeness
clog <- igraph::closeness(cnetwork3, normalized = TRUE) 
# make the NA a zero
clog[which(is.na(clog))] <- 0
# plot node size proportional to closeness 
# make one plot window
par(mfrow=c(1,1))
# plot
plot(cnetwork3, vertex.size = clog^2*10, vertex.label.dist = 2)
# Note: I transform closeness to amplify the visual differences.  I also move the labels away from the node centers with vertex.label.dist = 2.
```
...
V(cnetwork3)$color = "white" 
V(cnetwork3)$color = pal(max(igraph::degree(cnetwork3))+1)[igraph::degree(cnetwork3)+1] 
...
```{r errandomgraph}
# sample_gnp(node count, edge probaility)
ergp <- sample_gnp(40, .05)
# sample_gnm(node count, edge count)
...
```{r configmodel}
cnetwork3 <- thresh_graph(cnetwork, 3)
cnetconfig <- igraph::rewire(cnetwork3, igraph::keeping_degseq(niter = vcount(cnetwork3)*10))
...
par(mar=c(1,1,1,1))
# plot cnetwork3
fixlayout <- igraph::layout_nicely(cnetwork3)
plot(cnetwork3, vertex.label.dist = 2, main = "Original", layout = fixlayout)
# plot rewired cnetwork 
plot(cnetconfig, vertex.label.dist = 2, main = "Reconfigured", layout=fixlayout)
